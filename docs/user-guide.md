...
4. 左侧导航：在接口列表、断点列表、调用记录、历史会话和设置之间切换。
...
工具左侧“设置”页用于配置被调试业务服务的断点开关地址，包括服务 IP/Host、端口、断点启用接口和请求超时。配置会自动保存到应用目录下的 `data/settings.json`；点击“开始调试 / 停止调试”时，断点后端会按该配置向业务服务发送 `POST {"enabled": true|false}`。
