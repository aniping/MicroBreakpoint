...
5. 在“调用记录”和“接口列表”中查看当前 Session 的动态记录；调用记录表格按 `objectName` 分组并铺满分组宽度，默认八列为“序号 / 命令 / 槽位 / 参数摘要 / 状态 / 耗时 / 断点 / 接口”，断点命中和接口登记状态分别独立展示。“线程名”“调用时间”等长字段在右侧详情以 Tab 和信息卡片展示。接口样本页使用横向样本选择和整宽 Payload 查看区，调用详情右侧面板保留更宽的 Tab 栏和按钮操作区。
