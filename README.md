...
4. “调用记录”页应出现当前 Session 的调用记录，并按 `objectName` 可折叠分组展示 `cmdName`、`slotId` 和 `params` 摘要；每个分组内支持搜索、状态过滤、断点命中过滤、表头排序和列宽拖拽，页面底部显示全局总计。
5. “已发现接口”页应按 `objectName` 可折叠分组展示接口、参数样本数、唯一键、断点状态和业务别名；接口卡片采用主信息、统计和操作三列布局，别名默认文本展示，点击“编辑”后再进入输入态。
6. 从已发现接口创建命令断点，或从调用记录创建命令断点 / 参数快照断点；“断点管理”页按 `objectName` 可折叠分组展示启用状态、命中范围、来源和命中记录，卡片右侧保留固定操作区。
