...
5. “已发现接口”页应按 `objectName` 可折叠分组展示接口、参数样本数、唯一键、断点状态和整数平均耗时；接口卡片采用主信息、统计和固定操作列布局，操作按钮统一对齐。
6. “已发现接口”右侧详情可查看同一接口的不同参数样本。参数样本是相同 `objectName + cmdName + slotId` 下不同参数快照的去重结果，可用于回看历史入参，也可从对应调用创建参数快照断点。
7. 从已发现接口创建命令断点，或从调用记录创建命令断点 / 参数快照断点；“调用记录”页右侧详情底部展示当前 Session 的断点列表，支持启用、禁用和删除；“断点管理”页按 `objectName` 可折叠分组展示启用状态、命中范围、来源和命中记录，卡片右侧保留固定操作区。
8. 再次触发同一业务接口，命中断点后调用状态应变为 `paused`，顶部会出现暂停提示条。
9. 点击“继续执行”或“继续全部”后 Java 请求恢复，状态最终变为 `finished`。
10. 点击“停止调试”后，再触发 Java 接口不会新增调用记录；再次开始同一 Session 时会继续追加数据。
