...
`.mbrec` 文件用于归档单个 MicroBreakpoint Session。桌面端历史会话页支持导出归档名称和备注，导入时会先停止当前调试、释放 paused 调用，再创建并打开一个新的本地 Session；导入不会合并到当前 Session。

后端通过 `archiveId` 防止重复导入。重复导入会返回已有 Session ID，桌面端可打开该 Session。导入时可选择导入后锁定接口，主页也提供常驻锁定接口开关。锁定接口只阻止新接口自动加入“已发现接口”，不会影响调用记录保存、断点判断或暂停逻辑；未登记调用会在调用记录中标记，并可手动加入已发现接口。
