...
桌面端 Payload 预览区支持横向和纵向滚动，长 JSON 不会撑进调用列表或日志；后端会生成结构化裁剪后的合法 JSON preview，避免直接截断字符串导致无法格式化。`LargePayloadViewer` 同时支持 `callId+type` 和 `payloadId` 两种读取来源，“加载全部”会从 offset 0 拼接完整内容到界面，只有 preview 覆盖完整 payload 时才显示已加载全部，大文件建议直接导出查看。搜索框只触发后端完整 payload 搜索，结果使用浮层显示，不占用 JSON 展示区高度。
